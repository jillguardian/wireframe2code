...
def adaptively_binarize(image, block_size, delta):
    ...
    def binarize_by_block(image, block_size, delta):
        ...
        for row in range(0, image.shape[0], block_size):
            for col in range(0, image.shape[1], block_size):
                index = (row, col)
                index = get_block_index(image.shape, index, block_size)
...
def soften_binarization(image, mask, block_size):
    ...
    def combine_blocks(image, mask, block_size):
        ...
        for row in range(0, image.shape[0], block_size):
            for col in range(0, image.shape[1], block_size):
                index = (row, col)
                block_index = get_block_index(image.shape, index, block_size)
...
class Capture:
    ...
    def __preprocess(self):
        ...
        gamma_corrected = adjust_gamma(self.image)
